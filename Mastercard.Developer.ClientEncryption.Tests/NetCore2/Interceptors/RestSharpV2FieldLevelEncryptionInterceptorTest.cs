...
namespace Mastercard.Developer.ClientEncryption.Tests.NetCore.Interceptors
{
    ...
    public class RestSharpV2FieldLevelEncryptionInterceptorTest
    {
        ...
        public void TestInterceptRequest_ShouldEncryptRequestPayloadAndAddEncryptionHttpHeaders_WhenRequestedInConfig()
        {
            ...
            Assert.IsFalse(encryptedPayload.Contains("foo"));
            Assert.IsTrue(encryptedPayload.Contains("encryptedFoo"));
            Assert.AreEqual(encryptedPayload.Length.ToString(), request.Parameters.TryFind("Content-Length").Value);
            Assert.IsNotNull(request.Parameters.TryFind("x-iv"));
            Assert.IsNotNull(request.Parameters.TryFind("x-encrypted-key"));
            Assert.AreEqual("SHA256", request.Parameters.TryFind("x-oaep-padding-digest-algorithm").Value);
            Assert.AreEqual("80810fc13a8319fcf0e2ec322c82a4c304b782cc3ce671176343cfe8160c2279", request.Parameters.TryFind("x-encryption-certificate-fingerprint").Value);
            Assert.AreEqual("761b003c1eade3a5490e5000d37887baa5e6ec0e226c07706e599451fc032a79", request.Parameters.TryFind("x-encryption-key-fingerprint").Value);
        }
        ...
        public void TestInterceptResponse_ShouldDecryptResponsePayloadAndRemoveEncryptionHttpHeaders_WhenRequestedInConfig()
        {
            ...
            Header[] param =
            {
                ...
                new Header("x-oaep-padding-digest-algorithm", "SHA256"),
                new Header("x-encryption-key-fingerprint",
