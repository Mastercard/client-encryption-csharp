...
{
    public class JweObject : EncryptionParams
    {
        ...
        {
            byte[] unwrappedKey = UnwrapSecretKey(config, Base64Utils.URLDecode(EncryptedKey), "SHA-256");
        ...
        public static string Encrypt(JweConfig config, String payload, JweHeader header)
        {
            ...
            byte[] encryptedSecretKeyBytes = WrapSecretKey(config, cek);
